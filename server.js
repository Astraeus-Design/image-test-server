...
// connect app to mongo image database
...
const myImageModel = mongoose.model("imageCollection", myImageSchema);
...
async function getItems(req, res) {
  try {
    const items = await myImageModel.find({});
...
// handler for receiving new image request and data from client
...
async function postHandler(req, res) {
  ...
  const schema = {
    ...
    properties: {
      ...
      imageReg: { type: "string" },
      imageThumb: { type: "string" },
      title: { type: "string" },
      description: { type: "string" },
    },
    required: ["searchTerm", "imageReg", "imageThumb", "title", "description"],
  };
  ...
  // if not valid send error response otherwise update database and send response with all items in collection
    ...
    try {
      const { searchTerm, imageReg, imageThumb, title, description } =
        clientData;

      const newItem = await myImageModel.create({
        searchTerm,
        imageReg,
        imageThumb,
        title,
      ...
      try {
        const images = await myImageModel.find({});
...
async function idHandler(req, res) {
  ...
  try {
    const clientData = req.body;
    const { searchTerm, imageReg, imageThumb, title, description } = clientData;

    const idStr = req.params.id;
    //const idStrObj = mongoose.Types.ObjectId(idStr);
    console.log(idStr);

    const queryResp = await myImageModel.findOneAndUpdate(
      { _id: idStr },
      {
    ...
    if (!queryResp) {
      ...
      try {
        const queryResp2 = await myImageModel.find({});
        if (!queryResp2) {
...
async function deleteHandler(req, res) {
  ...
  try {
    const retVal = await myImageModel.findByIdAndDelete(idStr);
    if (!retVal) {
      console.log(err);
      ...
      const findVal = await myImageModel.find({});
      if (!findVal) {
        console.log(err);
        res.status(500).send("error deleting item");
      } else {
...
app.get("/myimages", getItems);
app.post("/myimages", postHandler);
app.put("/myimages/:id", idHandler);
app.delete("/myimages/:id", deleteHandler);
